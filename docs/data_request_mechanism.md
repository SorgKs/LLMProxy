...
data = {
    ...
    "EOF": true,            # True если файл передан полностью
    "content": {
...
2. Итерирует по всем tool calls в обратном порядке (от конца к началу)
   - Если `function.name == "write_file"` или `"apply_diff"` и их путь совпадает с искомым (target_path) — **останавливает итерацию** (модификация затрагивает искомый файл, последующие вызовы неактуальны)
...
6. **Определяет полноту файла:**
   - Использует параметры `offset` и `limit` для анализа прочитанного диапазона
   - Если `offset=1` и `content` непустой — это полный файл (`EOF=True`)
   - Если `offset>1` — это частичное чтение
   - Если указан `limit` и количество возвращённых строк меньше `limit` — значит достигнут конец файла:
     * Считается `EOF=True` (указывает на то, что конец файла прочитан)
     * Номер последней полученной строки становится значением `line_count`
     * Важно: `EOF=True` не гарантирует полного контента (отсутствия пропусков).
       Полный контент = все строки заполнены + `EOF=True`
   - Если `limit` равно количеству строк — возможно полное прочтение или промежуточное срез
   - Если `limit` не указан:
     * При `offset=1` и непустом content — `EOF=True`, `line_count=количество_полученных_строк`
     * При `offset>1` — `EOF=False`, `line_count` оценивается как `offset + количество_полученных_строк - 1`
...
1. **Проверка наличия `data`** — если `data` пусто или `None`, возвращает `False`
2. **Проверка типа** — если `data["type"] != "file_content"`, возвращает `False`
3. **Проверка совпадения пути** — если `pending_info["path"]` не совпадает с `data["path"]`, возвращает `False`
4. **Проверка контента** — анализирует `data["content"]`:
   - Если `content` пустой словарь `{}` — возвращает `False`
   - Если `EOF=True` и нет пропусков в нумерации строк (все строки от 1 до N заполнены) — возвращает `True` (полный файл)
   - Если `EOF=False` — файл считается неполным, возвращает `False`
   - Если есть пропуски в нумерации (например, есть строки 1,3,5 но нет 2,4) — возвращает `False`
5. **Дополнительная проверка по line_count** — если `line_count` указан, проверяет соответствие количеству строк в `content`

### Возвращаемое значение

- `True` — данные представляют собой полный, непрерывный контент целевого файла
- `False` — данных недостаточно или они неполные
...
**Возвращает:** `True`, если данные представляют собой полный файл (контент не пуст и непрерывен)

#### `check_function_sufficiency()`

Проверяет, содержит ли `data` полное определение требуемой функции.
Используется для определения, достаточно ли прочитанного контента файла для успешного преобразования `function_replace` → `apply_diff`.

### Логика проверки

Функция выполняет следующие шаги:

1. **Базовая валидация `data`** — проверяет наличие и корректность `data`:
   - `data` не пусто и не `None`
   - `data["type"] == "file_content"`
   - `data["path"]` совпадает с `pending_info["path"]`
   - Если базовые проверки не проходят — возвращает `False`

2. **Извлечение контента** — получает словарь строк из `data["content"]`

3. **Проверка заполненности** — если `content` пустой или `None` — возвращает `False`

4. **Проверка заполненности нумерации** — пропуски в номерах строк делают данные недействительными (возвращает `False`)
5. **Поиск определения функции** — ищет начало целевой функции:
   - Использует `pending_info["function_name"]` для поиска шаблона `def {function_name}(`
   - Если определение не найдено — возвращает `False`

6. **Алгоритм определения полноты функции**:
   - Находит базовый отступ (indent) у строки с `def`
   - Идёт последовательно по строкам тела функции
   - Если текущая строка непустая и её отступ **меньше или равен** базовому — конец тела функции (предыдущая строка была последней)
   - Если в нумерации обнаруживается пропуск строки — возвращает `False`
   - Если достигнут конец данных (`EOF=True` и `line_count` соответствует последней строке) — тело целиком
   - Если достигнут конец данных при `EOF=False` — возвращает `False` (файл обрезан, тело не полное)

7. **Проверка целостности**:
   - Функция может быть полной даже если файл прочитан частично (срез)
   - Главное: сигнатура и тело должны быть полностью в `content`
   - Не допускается обрезка по краям (текст не начинается/заканчивается внутри тела)
   - Пустые строки внутри/в конце функции допускаются

### Возвращаемое значение

- `True` — `data` содержит полное определение искомой функции (текст файла может быть частичным)
- `False` — данных недостаточно, функция неполная или не найдена
...
**Аргументы:**
- `data` — словарь с данными, передаваемый в `process()` (тип `file_content`)
- `pending_info` — информация из `_pending_function_replace` (содержит `function_name`, `path`)

**Возвращает:** `True`, если содержит полное определение искомой функции (файл может быть прочитан частично)

#### Условие вызова `function_replace`
...
1. Сформировать `data` с полями: `type`, `path`, `line_count`, `EOF`, `content`
...
`_extract_file_content_from_read_file_calls()` должна устанавливать `EOF=True`, если при использовании `limit` возвращённое количество строк меньше запрошенного (достигнут конец файла), либо если `offset=1` и файл прочитан полностью. Значение `line_count` определяется как номер последней полученной строки при достижении `EOF`, либо оценивается на основе доступных данных.
...
def process(self, answer, request_body: dict = None, data: dict = None) -> Optional[dict]:
    ...
    Args:
        ...
        data: Дополнительные данные, переданные из proxy.py
              Формат: {"type": "file_content", "path": "...", "line_count": N, "EOF": T, "content": {...}}
