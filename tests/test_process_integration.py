# tests/test_process_integration.py
import json
import os
import sys
import logging
...
class TestProcessIntegration:
    """Интеграционный тест AnswerProcessor.process() на основе case"""
    ...
    def create_answer(self, tool_call: dict) -> Mock:
        """Создает Answer с tool_call"""
        response_data = {
            "response": {
                "choices": [{
                    "message": {
    ...
    def test_process_with_data_all_cases(self):
        """Комплексный тест process() со всеми case"""
        test_cases = self.load_test_cases()
        if not test_cases:
            return

        print(f"\n{BOLD}{'='*80}{RESET}")
        print(f"{BOLD}КОМПЛЕКСНЫЙ ТЕСТ process() С ДАННЫМИ{RESET}")
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            # Формируем tool_call как от LLM
            diff_input = test_case["body"].get("diff", "")
            ...
            # Создаем answer
            answer = self.create_answer(tool_call)
            ...
            # Вызываем process
            result = self.processor.process(answer, data)

            # Проверяем результат
            if result is None:
                # process завершился без возврата action - проверяем tool_calls
                if answer.tool_calls:
                    args_str = answer.tool_calls[0]['function']['arguments']
                    parsed = json.loads(args_str)
                    actual_diff = parsed.get("diff", "")
                else:
                    actual_diff = ""
                ...
                # process вернул action (например, request_file)
                print(f"{YELLOW}  ⚠️ Возвращен action: {result}{RESET}")
                failed += 1
    ...
    def test_process_without_data(self):
        """Тест process() без данных (data=None)"""
        print(f"\n{BOLD}{'='*60}{RESET}")
    ...
    def test_process_read_file_adds_mode(self):
        """Тест что process() добавляет mode=slice к read_file"""
        print(f"\n{BOLD}{'='*60}{RESET}")
